    ...
    methods
        ...
        function s = get_links(self, rho)
            ...
            for i = 1:nTubes
                ...
                transitionPoints(2*i) = rho(i) + tubes(i).d
            end
        ...
        function [phi,K] = calculate_phi_and_kappa(self, theta, rho)
            for tubeIdx = 1:self.num_tubes
                E = self.tubes(tubeIdx).E;
                OD = self.tubes(tubeIdx).od;
                ID = self.tubes(tubeIdx).id;
                k = self.tubes(tubeIdx).k;
                alpha = theta;
                rho = rho;
                d = self.tubes(tubeIdx).d;
            ...
            chi_all = zeros(1, num_tubes)
            gamma_all = zeros(1, num_tubes)
